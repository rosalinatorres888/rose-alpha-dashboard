...
A personal AI-powered market intelligence terminal built around two analyst personas: **Rosa** (investment analyst, market research, agentic tool use) and **Max Volatility** (fictional chaos-event predictor, volatility signals). Runs entirely client-side with live market data and direct Claude API calls.
...
| Tab | Description |
|-----|-------------|
| **Market** | Live prices, Rosa research chat, sector performance, asset allocation, earnings watch, correlation alerts |
| **Charts** | Candlestick charts per asset, AI technical analysis, Ask Rosa deep dive |
| **News** | Live market news feed, Ask Rosa about headlines |
...
| **🔥 Chaos Alpha** | Max Volatility persona event predictor, portfolio impact panel, live Chaos Index, prediction history with P&L scoring |
| **📓 Journal** | Conviction Journal — every Rosa signal auto-captured and scored vs real price at 7d/30d |
| **💥 Stress** | Portfolio Stress Replay — simulate current holdings against 6 historical crashes |
| **🗒 Trades** | Natural language trade log with bias tagging, Rosa monthly audit |
...
- Ask Rosa button: routes any chaos event directly to Rosa for regime-aware analysis
...
- Regime context injected into every Rosa prompt — analysis shifts automatically based on macro environment
- Persisted to `rose_macro_regime` localStorage with 6h TTL; manual ↻ refresh button

### Conviction Journal
- Auto-captures every bullish/bearish/neutral signal Rosa emits (chat, morning brief, agent mode)
...
- Rosa narrates a 100–140 word lesson citing your exact numbers, regime-aware
...
- Brief button fires a pre-earnings Rosa research prompt with full position context
...
- Ask Rosa button routes divergence analysis to research tab
...
- Bias Audit button: sends last 20 trades to Claude, Rosa returns a 150–180 word pattern analysis covering dominant bias, what's working, recurring behavioral trap, and one discipline rule
...
- Ask Rosa (deep dive): builds full investment thesis with web search, current price, chart period, and any existing technical summary
...
| [Tavily](https://tavily.com) | Web search for Rosa agent tool use |
...
2. On first load, enter your API keys in the modal:
   ...
   - **Tavily** — for Rosa web search tool (optional but recommended)
...
| Key | Contents | TTL |
|-----|----------|-----|
| `rose_conviction_journal` | Rosa signal history with 7d/30d outcomes | permanent |
...
| `rose_research_threads` | Multi-turn Rosa research threads | permanent |
